...
soul <- mutate(soul, QD1gr = cut(QD1, breaks = c(0, 30, 40, 50, 60, 70, 
                                                 80, 99), include.lowest = T), QD2gr = cut(QD2, breaks = c(0, 10, 20, 
                                                                                                           50, 99), include.lowest = T), prop.res = QD2/QD1, prop.gr = cut(prop.res, 
                                                                                                                                                                           breaks = c(0, 0.25, 0.5, 1, 5.5), include.lowest = T))
soul$prop.res[soul$QD1 == 0] <- NA
demo.red <- c(colnames(soul.red)[c(6, 51:62)], "QD1gr", "QD2gr", "prop.res", 
              "prop.gr")
# Main reduction of data: Only variables with less than 25% of missing,
# and complete cases withion those vars.
aux <- apply(is.na(soul[, c(orig.red, demo.red, "CCE", "CCEGRP2")]), 1, 
             sum)
condi1 <- aux == 0
soul.red <- soul[condi1, c(orig.red, demo.red, "CCE", "CCEGRP2", "QSB", 
                           "YEAR", "WEIGHT", "PROJWT")]
...
reco.aux <- NULL
for (i in 1:length(orig.red)) reco.aux <- rbind(reco.aux, reco.fun(soul.red[, 
                                                                            i]))
...
soul.red2 <- data.frame(reco.aux, reco.demo, soul.red[, c("CCE", "YEAR", 
                                                          "CCEGRP2", "QSB", "WEIGHT", "PROJWT")])
