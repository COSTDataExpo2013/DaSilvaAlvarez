...
The Knight survey data contains missing information. The proportion of missing values was computed for each observation (by variables) and for each variable (by cases). Table \ref{tab1} shows the quantiles of the proportion of missing values computed by case and computed by variable. Focusing on the proportions of missing values by variables, there is 19\% of missing record for the most complete response. This may be explained by the fact that 3 years of data was combined although some variables only appeared in 1 or 2 years. On the other hand, focusing on the proportion of missing values by casess, most of the variables show a reasonable proportion of missing values (the median is 12\%) while there are a few variables with really high proportions of missing cases. 
...
Product plot is a very flexible framework \citep{wickham2011product}, which incorporate displays like bar charts, spine plots \citep{hummel1996linked} and mosaic plots \citep{hartigan1981mosaics,friendly1994mosaic}. This framework is designed for study relations among categorical variables. However, it is easily adapted when the variables are continuous simply by binning variables.
...
@
\caption{Relation between community attachment and k-means cluster solution. Cluster 1 shows the higest attachment proportion while cluster 2 the lowest. Attachment distribution is stable within each grupo amont years. \label{clus1}}
\end{figure}
...
<<race, echo=FALSE, dependson=c( 'data' ), message=FALSE, fig.height=4>>=
...
# b <- merge(census.data,tab[,c('QSB','diver','statistic.raceWhite') ], by='QSB') # why?
# rho.div <- with(tab, round(cor(diver, statistic.CCE),2)) # what is used for??
...
# 
# ggplot(data = tab, aes(x = diver, y = logit(statistic.CCEGRP2Attached))) + geom_point(size = I(5)) + theme(axis.text.y = element_text( size = I(10)) ) + labs( y = 'Proportion of Attach (logit scale)', x = 'Race diversity index') + geom_smooth(se = FALSE, method = 'lm', size = I(1))
