...
The main data set used in this paper was provided by the Knight Foundation, consisting of collected data from 47821 people over three years (2008, 2009 and 2010) in 26 communities across the United States. The data is available at http://streaming.stat.iastate.edu/dataexpo/2013/ as part of the 2013 Data Expo competition of the American Statistical Association. Additional details on the 2013 Data Expo can be found in \cite{soul2}.
...
As mentioned earlier, information from ACS corresponds to tables of comunity totals (e.g. Total of 18 years old women in each community). These variables were already expanded by Census Bureau, so there is no need to consider sampling weights again. For instance, proportions of age group by gender shown in Figrue \ref{age} are computed directly from ACS data without explicit use of sampling weights. 

When using data from Knight Foundation surveys, the sampling weights need to be taken into
account. There are two variables representing sampling weights that might be used considering if data from many communities are combined (PROJWT) or computations are done for each community separatedly (WEIGHT) \citep{full2010}. For instance, Figure \ref{pp1} includes data from all communities toghether so PROJWT is used as sampling weight, meanwhile Figure \ref{comu} shows proportions of attachment computed separatedly within each community  and therefore sampling weights correspond to WEIGHT variable. Additionally, when all three year survey data are used jointly, sampling weights are divided by three so the total aggregate of each variable still can be consider an estimate of the comunity total.
...
@
\caption{Dendrogam of communities based on importance measures. Communities close to each other have similar structure of key drivers of attachment. \label{dendro}}
\end{figure}
