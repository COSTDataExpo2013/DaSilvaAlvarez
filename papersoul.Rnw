...
prodplot(aux, ~CCEGRP2+Q6A+QSB, divider=c("vspine","hspine",'hspine'),subset=level == 2)+aes(fill=CCEGRP2) + facet_wrap( facet=~QSB, scale='free',ncol=7) + theme( legend.title=element_blank(),axis.text.x=element_blank(),axis.text.y=element_blank(),  axis.title.y=element_blank(),axis.title.x=element_blank(),legend.position=c(.9,0.1) ) + scale_fill_brewer(type='seq', palette=1)
@
