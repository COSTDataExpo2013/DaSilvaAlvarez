...
cors <- census.data %>% 
  ...
  filter(rank(cor.att) %in% c(1:10, 52:61) ) %>% 
...
Beauty = Q7A, Roads = Q7B, Housing = Q7C, Night.Life = Q8C, Minorities = Q8E, Inmigrants = Q8B, Gay = Q7H, Safe = Q18, Colleges = Q7G, Friends = Q24, Clusters = cl.id) %>% 
 gather(vars, value, -Clusters,-YEAR) %>% 
  ggplot(aes(x = vars, y = value, color = Clusters, shape = Clusters), size = I(3)) + labs(x = "", y = "") +  geom_point(size = I(2))  + geom_line(aes(group = Clusters)) + 
