...
Statistical analysis of data from survey need to make a correct use of sampling weights. Depending of the source of information the type of statistical goal, different sampling weights are used in several statistical methods or summaries.
...
account. There are two variables representing sampling weights that should be used depending if data from many communities are used jointly (PROJWT) or computations are done for each community separatedly (WEIGHT) \citep{full2010}. For instance, Figure \ref{pp1} includes data from all communities toghether so PROJWT is used as sampling weight, meanwhile Figure \ref{comu} shows proportions of attachment computed separatedly within each community  WEIGHT variable is used as sampling weight. Additionally, when all three year survey data are used jointly, sampling weights are divided by three so the total aggregate of each variable still can be consider an estimate of the comunity total.
...
The variable 5.years.ago represents the perception of the community improvement, when comparing the community today with 5 years ago. Finally, the Meet.People variable indicates if the community is a good place to meet people and make friends.
\begin{figure}[hbpt]
