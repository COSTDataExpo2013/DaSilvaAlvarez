...
Data from ACS are community variable, computed using sampling weights by Census Bureau. 
When using data from Knight Foundation surveys, the sampling weights need to be taken into
account, if the data comes from only one year then the sampling weight is provided
in the data, when all three year survey data are used, sampling weights are divided by three 
so the total aggregate of each variable still can be consider an estimate of the community
total.
...
@
\caption{Relation between community attachment and k-means cluster solution. Cluster 1 shows the highest attachment proportion while cluster 2 the lowest. Attachment distribution is stable within each group among years. \label{clus1}}
\end{figure}
...
soul %>% select(race, CCEGRP2, CCE, QSB,PROJWT, YEAR) %>% 
  filter(!is.na(PROJWT), YEAR!=2010)  %>% 
  mutate(wt = PROJWT/2) %>% 
