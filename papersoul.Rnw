...
Data from ACS are comunity variable, computed using sampling weights by Census Bureau. 
...
so the total aggregate of each variable still can be consider an estimate of the comunity
total.
...
\begin{figure}[hbpt]
<<econ, dependson=c('data'), echo=FALSE,message=FALSE,warning=FALSE, fig.height=4>>=
svydesign(ids =~ 1, strata =~ QSB, weights =~ PROJWT, data = filter(soul,!is.na(PROJWT))) %>% 
  ...
  mutate(variable =  factor(variable, labels = c('Attachment-Economy correlation', 'GDP Rate')
...
# des.wt.year <- svydesign(ids =~ 1, strata =~ QSB, weights =~ PROJWT, data = filter(soul,!is.na(PROJWT)))
# vars  <- svyby( ~ ECONOMY + CCE, ~ YEAR, des.wt.year, svyvar, keep.var = TRUE, na.rm = T)
# vars <- mutate(vars[, 1:5], rho = with(vars, V2/sqrt(V1*V4)), gdp = c(-0.4,-3.5,3.0))
# 
# rhos.m <- gather(vars, variable, value,rho,gdp) %>%
#   select(YEAR, variable, value) %>%
#   mutate(variable =  factor(variable, labels = c('Attachment-Economy correlation', 'GDP Rate')
# ))
# 
# ggplot(data = rhos.m, aes(x = YEAR, y = value)) +
#   labs(x = "", y = "", title = 'Evolution of Attachment-Economy correlation and GDP') +
#   geom_line() + geom_point() +
#   facet_grid(facets = variable~., scales = 'free') +
#   ggtitle('Evolution of Attachment-Economy correlation and GDP')
