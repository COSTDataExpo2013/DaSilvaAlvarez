...
# var.cl2 <- colnames(census.data) %in% c("aveageF","prop1821", "Bachelor.s.degree.or.higher." , "gini" , "own.rate" , "afri.prop",'prop.new.entry','un.rate',"prop.no.workers" )
d2 <- census.data %>% select(aveageF, prop1821, Bachelor.s.degree.or.higher., gini , own.rate , afri.prop, prop.new.entry, un.rate, prop.no.workers ) %>% dist()
...
nn <- c('cl.2', 'Age.Fem', '1821.Yr','Bachelor', 'Unemp','Gini','Own.rate', 'Afri.pr', 'No.workers', 'New.entry' )

census.data %>%
  mutate(cl.2 = cutree(hc2, 4) ) %>% 
  select( c(colnames(census.data)[var.cl2],'cl.2' ) ) %>% 
  group_by(cl.2) %>%
...
\begin{figure}[hbpt]
<<corplot, echo=FALSE, dependson=c('data'),cache=TRUE>>=
...
lab <- c('Family','Refer','Perfect','Children','Leader','Proud', 'Lead. Int.','Talented','Job','Meet.People','
         5year.ahead', 'School','Reputation','5year.ago', 'Care', 'Senior','Economy','Satisfaction','Health',
         'Find.Job', 'Parks','Beauty','Roads','Housing','Night.Life', 'Minorities', 'Inmigrants', 'Gay', 'Safe','Colleges', 'Friends')
...
qplot(data = aux2, var2, value, color = Clusters,shape = Clusters,xlab = '', ylab = '',size = I(3)) +geom_line(size = I(.5)) + facet_wrap(~YEAR,nrow = 3) + theme(axis.text.x = element_text(angle = 90, size = I(15)), legend.position = 'bottom' ) + scale_x_discrete(breaks = 1:31, labels = lab) + scale_color_brewer(type='qual', palette='Paired')
@
