...
lifevar %>% data.frame() %>%  select(-starts_with('se')) %>%
  ggplot(data =., x=QSB,ymax=rho,ymin=0, xlab='',geom='linerange', ylab='Correlation',size=I(2))  + theme(axis.text.y = element_text( size=I(10) ),axis.text.x = element_text(size=I(10), angle=90),axis.title.x = element_text(size=I(25)))
@
